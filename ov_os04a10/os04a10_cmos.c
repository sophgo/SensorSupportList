...
SNS_COMBO_DEV_ATTR_S *g_pastOs04a10ComboDevArray[VI_MAX_PIPE_NUM] = {CVI_NULL};
...
#define OS04A10_SENSOR_SET_COMBO(dev, pstCtx)   (g_pastOs04a10ComboDevArray[dev] = pstCtx)
#define OS04A10_SENSOR_GET_COMBO(dev, pstCtx)   (pstCtx = g_pastOs04a10ComboDevArray[dev])
...
ISP_SNS_COMMADDR_U g_aunOs04a10_AddrInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cAddr = 0},
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	case WDR_MODE_NONE:
		...
		if (pstSnsState->u8ImgMode == OS04A10_MODE_1440P30_SLAVE_WDR)
			pstSnsState->u8ImgMode = OS04A10_MODE_1440P30_2L_SLAVE_10BIT;
	...
	case WDR_MODE_2To1_LINE:
		...
		if (pstSnsState->u8ImgMode == OS04A10_MODE_1440P30_2L_MASTER_10BIT) {
			...
			CVI_TRACE_SNS(CVI_DBG_INFO, "WDR_MODE_2To1_LINE 1440p 2L MASTER mode(60fps->30fps)\n");
		}
		if (pstSnsState->u8ImgMode == OS04A10_MODE_1440P30_2L_SLAVE_10BIT) {
			pstSnsState->u8ImgMode = OS04A10_MODE_1440P30_SLAVE_WDR;
			CVI_TRACE_SNS(CVI_DBG_INFO, "WDR_MODE_2To1_LINE 1440p 2L SLAVE mode(60fps->30fps)\n");
		}
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = g_aunOs04a10_AddrInfo[ViPipe].s8I2cAddr;
			...
			if (OS04A10_RES_IS_1440P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				if (pstSensorImageMode->u8LaneNum == 2) {
					if (pstSensorImageMode->u8EnableMaster == ISP_SNS_MASTER_BY_FSYNC) {
						u8SensorImageMode = OS04A10_MODE_1440P30_2L_MASTER_10BIT;
					} else if (pstSensorImageMode->u8EnableMaster == ISP_SNS_SLAVE_BY_FSYNC) {
						u8SensorImageMode = OS04A10_MODE_1440P30_2L_SLAVE_10BIT;
					} else if (pstSensorImageMode->u8EnableMaster == ISP_SNS_NORMAL_MODE) {
						u8SensorImageMode = OS04A10_MODE_1440P30_2L_10BIT;
					}
				} else {
			...
			if (OS04A10_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				...
				if (pstSensorImageMode->u8LaneNum == 2) {
					if (pstSensorImageMode->u8EnableMaster == ISP_SNS_MASTER_BY_FSYNC) {
						...
						u8SensorImageMode = OS04A10_MODE_1440P30_SLAVE_WDR;
					}
				} else {
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	OS04A10_SENSOR_GET_CTX(ViPipe, pstSnsState);
	OS04A10_SENSOR_GET_COMBO(ViPipe, pstRxAttrSrc);
	...
	pstRxAttr->img_size.start_x = g_astOs04a10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32X;
	pstRxAttr->img_size.start_y = g_astOs04a10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.s32Y;
	pstRxAttr->img_size.active_w = g_astOs04a10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Width;
	pstRxAttr->img_size.active_h = g_astOs04a10_mode[pstSnsState->u8ImgMode].astImg[0].stWndRect.u32Height;
	...
	pstRxAttr->img_size.max_width = g_astOs04a10_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Width;
	pstRxAttr->img_size.max_height = g_astOs04a10_mode[pstSnsState->u8ImgMode].astImg[0].stMaxSize.u32Height;
	...
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
		...
		if (g_pastOs04a10[ViPipe]->u8ImgMode == OS04A10_MODE_1440P30_2L_10BIT ||
			...
			for (i = 3; i < MIPI_LANE_NUM + 1; i++) {
				pstRxAttr->mipi_attr.lane_id[i] = -1;
				pstRxAttr->mipi_attr.pn_swap[i] = 0;
			}
		} else {
			pstRxAttr->mipi_attr.dphy.hs_settle = 8;
		}
	} else {
...
static CVI_S32 sensor_patch_rx_attr(VI_PIPE ViPipe, RX_INIT_ATTR_S *pstRxInitAttr)
{
	...
	if (!g_pastOs04a10ComboDevArray[ViPipe]) {
		pstRxAttr = malloc(sizeof(SNS_COMBO_DEV_ATTR_S));
	} else {
		OS04A10_SENSOR_GET_COMBO(ViPipe, pstRxAttr);
	}
	memcpy(pstRxAttr, &os04a10_rx_attr, sizeof(SNS_COMBO_DEV_ATTR_S));
	OS04A10_SENSOR_SET_COMBO(ViPipe, pstRxAttr);
...
{
	if (g_pastOs04a10ComboDevArray[ViPipe]) {
		free(g_pastOs04a10ComboDevArray[ViPipe]);
		g_pastOs04a10ComboDevArray[ViPipe] = CVI_NULL;
	}
...
static CVI_VOID sensor_patch_i2c_addr(VI_PIPE ViPipe, CVI_S32 s32I2cAddr)
{
	if (OS04A10_I2C_ADDR_IS_VALID(s32I2cAddr))
		g_aunOs04a10_AddrInfo[ViPipe].s8I2cAddr = s32I2cAddr;
	else {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C addr input error ,please check [0x%x]\n", s32I2cAddr);
		g_aunOs04a10_AddrInfo[ViPipe].s8I2cAddr = OS04A10_I2C_ADDR_2;
	}
}
