...
#include "sc2336_cmos_param.h"

#define DIV_0_TO_1(a)   ((0 == (a)) ? 1 : (a))
#define DIV_0_TO_1_FLOAT(a) ((((a) < 1E-10) && ((a) > -1E-10)) ? 1 : (a))
#define SC2336_ID 35
#define SENSOR_SC2336_WIDTH 1920
#define SENSOR_SC2336_HEIGHT 1080
/****************************************************************************
 * global variables                                                            *
 ****************************************************************************/

ISP_SNS_STATE_S *g_pastSC2336[VI_MAX_PIPE_NUM] = {CVI_NULL};

#define SC2336_SENSOR_GET_CTX(dev, pstCtx)   (pstCtx = g_pastSC2336[dev])
#define SC2336_SENSOR_SET_CTX(dev, pstCtx)   (g_pastSC2336[dev] = pstCtx)
#define SC2336_SENSOR_RESET_CTX(dev)         (g_pastSC2336[dev] = CVI_NULL)

ISP_SNS_COMMBUS_U g_aunSC2336_BusInfo[VI_MAX_PIPE_NUM] = {
	[0] = { .s8I2cDev = 0},
	[1 ... VI_MAX_PIPE_NUM - 1] = { .s8I2cDev = -1}
};

CVI_U16 g_au16SC2336_GainMode[VI_MAX_PIPE_NUM] = {0};
CVI_U16 g_au16SC2336_L2SMode[VI_MAX_PIPE_NUM] = {0};

SC2336_STATE_S g_astSC2336_State[VI_MAX_PIPE_NUM] = {{0} };
...
/*****SC2336 Lines Range*****/
#define SC2336_FULL_LINES_MAX  (0xFFFF)

/*****SC2336 Register Address*****/
#define SC2336_SHS1_0_ADDR		0x3E00
#define SC2336_SHS1_1_ADDR		0x3E01
#define SC2336_SHS1_2_ADDR		0x3E02
...
{
	const SC2336_MODE_S *pstMode;
	ISP_SNS_STATE_S *pstSnsState = CVI_NULL;

	CMOS_CHECK_POINTER(pstAeSnsDft);
	SC2336_SENSOR_GET_CTX(ViPipe, pstSnsState);
	CMOS_CHECK_POINTER(pstSnsState);

	pstMode = &g_astSC2336_mode[pstSnsState->u8ImgMode];
	pstAeSnsDft->u32FullLinesStd = pstSnsState->u32FLStd;
	pstAeSnsDft->u32FlickerFreq = 50 * 256;
	pstAeSnsDft->u32FullLinesMax = SC2336_FULL_LINES_MAX;
...
static CVI_S32 cmos_fps_set(VI_PIPE ViPipe, CVI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
	...
	u32Vts = g_astSC2336_mode[pstSnsState->u8ImgMode].u32VtsDef;
	pstSnsRegsInfo = &pstSnsState->astSyncInfo[0].snsCfg;
	f32MaxFps = g_astSC2336_mode[pstSnsState->u8ImgMode].f32MaxFps;
	f32MinFps = g_astSC2336_mode[pstSnsState->u8ImgMode].f32MinFps;

	switch (pstSnsState->u8ImgMode) {
	case SC2336_MODE_1080P30:
		...
		u32VMAX = (u32VMAX > SC2336_FULL_LINES_MAX) ? SC2336_FULL_LINES_MAX : u32VMAX;
		break;
	default:
...
static CVI_S32 cmos_get_wdr_size(VI_PIPE ViPipe, ISP_SNS_ISP_INFO_S *pstIspCfg)
{
	const SC2336_MODE_S *pstMode = CVI_NULL;
	ISP_SNS_STATE_S *pstSnsState = CVI_NULL;
...
static CVI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, CVI_U8 u8Mode)
{
	...
	switch (u8Mode) {
	case WDR_MODE_NONE:
		pstSnsState->u8ImgMode = SC2336_MODE_1080P30;
		pstSnsState->enWDRMode = WDR_MODE_NONE;
		pstSnsState->u32FLStd = g_astSC2336_mode[pstSnsState->u8ImgMode].u32VtsDef;
...
static CVI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_SYNC_INFO_S *pstSnsSyncInfo)
{
	...
	if ((pstSnsState->bSyncInit == CVI_FALSE) || (pstSnsRegsInfo->bConfig == CVI_FALSE)) {
		...
		pstCfg0->snsCfg.unComBus.s8I2cDev = g_aunSC2336_BusInfo[ViPipe].s8I2cDev;
		...
		for (i = 0; i < pstCfg0->snsCfg.u32RegNum; i++) {
			...
			pstI2c_data[i].u8DevAddr = sc2336_i2c_addr;
			pstI2c_data[i].u32AddrByteNum = sc2336_addr_byte;
			pstI2c_data[i].u32DataByteNum = sc2336_data_byte;
		}
		...
		default:
			...
			pstI2c_data[LINEAR_AGAIN_0_ADDR].u32RegAddr = SC2336_AGAIN0_ADDR;
			pstI2c_data[LINEAR_DGAIN_0_ADDR].u32RegAddr = SC2336_DGAIN0_ADDR;
...
static CVI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
	...
	if (pstSensorImageMode->f32Fps <= 30) {
		if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
			if (SC2336_RES_IS_1080P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
				u8SensorImageMode = SC2336_MODE_1080P30;
			} else {
...
static CVI_S32 sensor_rx_attr(VI_PIPE ViPipe, SNS_COMBO_DEV_ATTR_S *pstRxAttr)
{
	...
	memcpy(pstRxAttr, &sc2336_rx_attr, sizeof(*pstRxAttr));

	pstRxAttr->img_size.width = g_astSC2336_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Width;
	pstRxAttr->img_size.height = g_astSC2336_mode[pstSnsState->u8ImgMode].astImg[0].stSnsSize.u32Height;
	if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
...
{
	SNS_COMBO_DEV_ATTR_S *pstRxAttr = &sc2336_rx_attr;
	int i;
...
static CVI_S32 cmos_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
	...
	pstSensorExpFunc->pfn_cmos_sensor_exit = sc2336_exit;
...
static CVI_S32 sc2336_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
	g_aunSC2336_BusInfo[ViPipe].s8I2cDev = unSNSBusInfo.s8I2cDev;
...
static CVI_S32 sensor_ctx_init(VI_PIPE ViPipe)
{
	...
	SC2336_SENSOR_GET_CTX(ViPipe, pastSnsStateCtx);
	...
	SC2336_SENSOR_SET_CTX(ViPipe, pastSnsStateCtx);
...
static CVI_VOID sensor_ctx_exit(VI_PIPE ViPipe)
{
	...
	SC2336_SENSOR_RESET_CTX(ViPipe);
}
...
static CVI_S32 sensor_register_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
	...
	stSnsAttrInfo.eSensorId = SC2336_ID;
...
static CVI_S32 sensor_unregister_callback(VI_PIPE ViPipe, ALG_LIB_S *pstAeLib, ALG_LIB_S *pstAwbLib)
{
	...
	s32Ret = CVI_ISP_SensorUnRegCallBack(ViPipe, SC2336_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AE_SensorUnRegCallBack(ViPipe, pstAeLib, SC2336_ID);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "sensor unregister callback function to ae lib failed!\n");
		return s32Ret;
	}

	s32Ret = CVI_AWB_SensorUnRegCallBack(ViPipe, pstAwbLib, SC2336_ID);
	if (s32Ret != CVI_SUCCESS) {
...
static CVI_S32 sensor_set_init(VI_PIPE ViPipe, ISP_INIT_ATTR_S *pstInitAttr)
{
	...
	g_au16SC2336_GainMode[ViPipe] = pstInitAttr->enGainMode;
	g_au16SC2336_L2SMode[ViPipe] = pstInitAttr->enL2SMode;

	return CVI_SUCCESS;
}

ISP_SNS_OBJ_S stSnsSC2336_Obj = {
	.pfnRegisterCallback    = sensor_register_callback,
	.pfnUnRegisterCallback  = sensor_unregister_callback,
	.pfnStandby             = sc2336_standby,
	.pfnRestart             = sc2336_restart,
	.pfnMirrorFlip          = sc2336_mirror_flip,
	.pfnWriteReg            = sc2336_write_register,
	.pfnReadReg             = sc2336_read_register,
	.pfnSetBusInfo          = sc2336_set_bus_info,
