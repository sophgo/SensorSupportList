...
int sc533hai_i2c_init(VI_PIPE ViPipe)
{
	...
	u8DevNum = g_aunSC533HAI_BusInfo[ViPipe].s8I2cDev;
	...
	ret = ioctl(g_fd[ViPipe], I2C_SLAVE_FORCE, sc533hai_i2c_addr);
	if (ret < 0) {
...
int sc533hai_read_register(VI_PIPE ViPipe, int addr)
{
	...
	if (sc533hai_addr_byte == 2)
		buf[idx++] = (addr >> 8) & 0xff;

	// add address byte 0
	buf[idx++] = addr & 0xff;

	ret = write(g_fd[ViPipe], buf, sc533hai_addr_byte);
	if (ret < 0) {
	...
	buf[0] = 0;
	buf[1] = 0;
	ret = read(g_fd[ViPipe], buf, sc533hai_data_byte);
	if (ret < 0) {
...
int sc533hai_write_register(VI_PIPE ViPipe, int addr, int data)
{
	...
	if (sc533hai_data_byte == 1) {
		buf[idx] = data & 0xff;
		idx++;
	}

	ret = write(g_fd[ViPipe], buf, sc533hai_addr_byte + sc533hai_data_byte);
	if (ret < 0) {
		CVI_TRACE_SNS(CVI_DBG_ERR, "I2C_WRITE error!\n");
		return CVI_FAILURE;
	}
...
void sc533hai_init(VI_PIPE ViPipe)
{
	...
	} else if (u8ImgMode == SC533HAI_MODE_1620P30_WDR) {
...
static void sc533hai_wdr_1620p30_init(VI_PIPE ViPipe)
{
	...
	printf("ViPipe:%d,===SC533HAI 1620P 30fps 10bit 2to1 WDR Init OK!===\n", ViPipe);
}
